...
- `a2aAgentProGearSalesId`: The client ID of the ProGearSales Agent, which will be used as the `iss` and `sub` claims in the JWT - `a2aOrgUrl`: The base URL of the Okta organization, used to construct the `aud` claim in the JWT.
...
{JWTBearerToken} - This is a JWT token signed by the ProGearSales Agent, which contains the necessary claims to authenticate the ProgearSales agent when calling ProGearInventory Agent. The JWT should include claims such as `sub`, `aud`, `iss`, and any other claims required by the authorization server for client authentication.
...
The PrivateKeyJWK is the same as used in Step 2 to generate the JWT Bearer Token for client authentication.
...
## 4. Use the obtained A2A access token from Step 3 to exchange for ProGearInventory Agent IDJAG that can be uded for token exchange to call InventoryMCP API
...
{JWTBearerToken} - This is a JWT token signed by the ProGearSales Agent, which contains the necessary claims to authenticate the ProgearInventory agent. The JWT should include claims such as `sub`, `aud`, `iss`, and any other claims required by the authorization server for client authentication.
...
--header 'Content-Type: application/x-www-form-urlencoded' \
--header 'Cookie: DT=DI1yB6vUwWpTAaNXVs9D7QrpQ; JSESSIONID=654B63E71D6AFAB8951EE30186EFCF8F' \
--data-urlencode 'grant_type=urn:ietf:params:oauth:grant-type:jwt-bearer' \
